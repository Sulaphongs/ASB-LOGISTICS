-- =====================================================================
-- ASB LOGISTICS — ຖານຂໍ້ມູນ
-- ບໍລິການຂົນສົ່ງ ແລະ ຮັບເຄື່ອງຈາກແອັບຈີນ (China -> Laos parcel forwarding)
-- =====================================================================
SET NAMES utf8mb4;

-- ---------------------------------------------------------------------
-- ບົດບາດ ແລະ ສິດການໃຊ້ງານ (Roles & permissions)
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS role_permissions (
    ...
    UNIQUE KEY uniq_role_menu (role_key, menu_key)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ---------------------------------------------------------------------
-- ຜູ້ໃຊ້ງານ (staff / admin) ແລະ session
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS users (
    ...
    role_key VARCHAR(50) DEFAULT 'staff',
...
-- ---------------------------------------------------------------------
-- ລູກຄ້າ (customers who order goods from China)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- ກົດເກນຄ່າຂົນສົ່ງ (pricing rules)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- ພັດສະດຸ / ອອເດີ (packages / parcels)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- ການຕັ້ງຄ່າ (key/value settings — company info, exchange rates)
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- SEED DATA — ຂໍ້ມູນເລີ່ມຕົ້ນ
-- =====================================================================
INSERT IGNORE INTO roles (role_key, role_name) VALUES
    ('admin', 'ຜູ້ດູແລລະບົບ'),
    ('staff', 'ພະນັກງານ');

-- default admin / password: admin123  (ປ່ຽນລະຫັດຜ່ານທັນທີຫຼັງຕິດຕັ້ງ!)
