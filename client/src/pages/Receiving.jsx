  ...
  return (
    <Layout title="ຮັບ-ສົ່ງ ສາງ">
      <div className="card mb-5">
        ...
        <p className="text-xs text-slate-500 mb-3">ພິມ ຫຼື ສະແກນ ລະຫັດຕິດຕາມ ASB ຫຼືເລກຕິດຕາມຈາກຮ້ານຈີນ</p>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 border border-slate-200 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-teal-500"
          />
      ...
      <div className="space-y-3">
        {searched && results.length === 0 && (
          <div className="card text-center text-slate-400">ບໍ່ພົບພັດສະດຸ</div>
        )}
        {results.map((p) => {
          ...
          return (
            <div key={p.id} className="card flex justify-between items-center flex-wrap gap-3">
              <div>
                ...
                <div className="text-sm text-slate-500">{p.customer_name} — {p.customer_phone} · {p.item_description || ''}</div>
              </div>
              <div className="flex gap-2">
                <Link to={`/packages/${p.id}`} className="btn btn-light btn-sm"><Eye className="w-4 h-4" /> ລາຍລະອຽດ</Link>
                {next && (
                  <button className="btn btn-primary btn-sm" onClick={() => advance(p.id, next)}>
